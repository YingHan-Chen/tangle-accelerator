...
static inline int process_find_txns_obj_by_tag_request(ta_http_t *const http, iota_client_service_t *const iota_service,
...
static inline int process_find_txns_by_tag_request(ta_http_t *const http, iota_client_service_t *const iota_service,
                                                   char const *const url, char **const out) {
  status_t ret;
  char *tag = NULL;
  ret = ta_get_url_parameter(url, 1, &tag);
...
static inline int process_find_txn_obj_single_request(ta_http_t *const http, iota_client_service_t *const iota_service,
                                                      char const *const url, char **const out) {
  status_t ret;
  char *hash = NULL;
...
static inline int process_find_txn_obj_request(ta_http_t *const http, iota_client_service_t *const iota_service,
...
static inline int process_get_tips_request(ta_http_t *const http, iota_client_service_t *const iota_service,
                                           char **const out) {
  status_t ret;
...
static inline int process_get_iri_status(ta_http_t *const http, iota_client_service_t *const iota_service,
                                         char **const out) {
  status_t ret;
...
static int ta_http_process_request(ta_http_t *const http, iota_client_service_t *const iota_service,
  ...
  if (api_path_matcher(url, "/mam/[A-Z9]{81}[/]?") == SC_OK) {
    ...
    return process_find_txn_obj_single_request(http, iota_service, url, out);
  } else if (api_path_matcher(url, "/transaction/object[/]?") == SC_OK) {
    if (payload != NULL) {
      return process_find_txn_obj_request(http, iota_service, payload, out);
    }
    return process_method_not_allowed_request(out);
    ...
    return process_get_tips_request(http, iota_service, out);
    ...
    return process_find_txns_by_tag_request(http, iota_service, url, out);
  } else if (api_path_matcher(url, "/tag/[A-Z9]{1,27}[/]?") == SC_OK) {
    return process_find_txns_obj_by_tag_request(http, iota_service, url, out);
  } else if (api_path_matcher(url, "/status[/]?") == SC_OK) {
    return process_get_iri_status(http, iota_service, out);
  }
#ifdef DB_ENABLE
...
status_t ta_http_start(ta_http_t *const http) {
  ...
  http->daemon = MHD_start_daemon(MHD_USE_EPOLL_INTERNAL_THREAD | MHD_USE_ERROR_LOG | MHD_USE_DEBUG,
                                  ...
                                  MHD_OPTION_THREAD_POOL_SIZE, http->core->ta_conf.thread_count, MHD_OPTION_END);
  if (http->daemon == NULL) {
    ta_log_error("%s\n", strerror(errno));
    return SC_HTTP_OOM;
  }
